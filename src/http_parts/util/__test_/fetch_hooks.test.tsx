...
describe("useHttpGetTrigger", () => {
  beforeEach(() => fetchMock.mockReset());

  test("正常系: fetch成功、データ型一致、Authorizagionヘッダあり", async () => {
    const schema = z.object({ id: z.number(), name: z.string() }).strict();
    type User = z.infer<typeof schema>;
    const data: User = { id: 1, name: "taro" };
    ...
    act(() => {
      result.current.getTrigger("example.com", "/path", schema);
    });
...
describe("useHttpPostTrigger", () => {
  beforeEach(() => fetchMock.mockReset());
  const payload = {};

  test("正常系: fetch成功、データ型一致、Authorizationヘッダあり", async () => {
    const schema = z.object({ id: z.number(), name: z.string() }).strict();
    type User = z.infer<typeof schema>;
    const data: User = { id: 1, name: "taro" };
    ...
    act(() => {
      result.current.postTrigger("example.com", "/path", payload, schema);
    });
